# -*- coding: utf-8 -*-
"""DMLAnalysisFINAL.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1DvOFXejLbqwzXzW_xiSjRsNA8sfWRRfm
...
The paper is located at: https://www.aeaweb.org/articles?id=10.1257/mac.1.2.55.

The data and code for replication can be accessed at: https://www.openicpsr.org/openicpsr/project/114047/version/V1/view.

- The paper analyses the effect of assassination attemps on changes in countries' political institutions (e.g., transition to democracy) and change in war status (e.g., emergence of conflicts and intensity).


- The fundamendal identification assumption is that the success of an assassination attempt is exogeneous of all unobserved causal factors. To make the hypothesis believable, the authors control for the type of weapon used in the assassination attempt and the number of attempts in a given country-year; indeed, different weapons have different lethality rates and success of an assassination depends directly on the number of attempts. The authors exclude other observable quantities based on their identical predictive power for success and failure.


- Two caveats are in order: the results obtained by the author are almost invariant to the inclusion or exclusion of the controls; the controls considered are quite low-dimensional. We thus expect to find similar results when using DoubleML methods.


- The authors consider the following linear speficiation:

$$
y_i=\beta SUCCESS_i+\gamma\mathbf{X}_i+\varepsilon_i
$$

where $y_i$ is the outcome of interest (institution change or war status change), $\bf X$ is a number of control variables (type of weapons, number of attempts), and $SUCCESS$ is a dummy variable taking value 1 if the attempt is successful and 0 otherwise. The key identification assumption for the average treatment effect of successful assassination attempt on $y_i$ is given by the standard exogeneity condition

$$
\mathbb{E}~[\epsilon_i\mid \mathbf{X},SUCCESS]=0~,
$$

in which case, we obtain

$$
\beta=\mathbb{E}~[y\mid SUCCESS=1,\mathbf{X}]-\mathbb{E}~[y\mid SUCCESS=0,\mathbf{X}].
$$
...
"""# Overview of the Stata OLS specifications

We describe below the Stata codes used by the author to output their main results.

### Fixed effect variables (i.e. controls):

```
fixedeffectvars = "weapondum* numserdum*"
```
...
### Main Stata OLS regressions (Table 5):
...
### Other Stata OLS regressions (Table 6):

Same regressions for some subsamples with conditions seriousattempt == 1 and either
...
### Robustness analysis (Table 8):

Regression with different subsamples and different fixed effects.
...
We reproduce the results in Jones and Olken with DoubleML. We consider a partially linear model of the form

$$
y = SUCCESS\beta+g_0(\mathbf{X})+\xi,\quad \mathbb{E}[\xi|SUCCESS,\mathbf{X}]=0,
$$
$$
SUCCESS = m_0(\mathbf{X})+V,\quad\quad\quad\quad\quad\quad\mathbb{E}[V|\mathbf{X}]=0.
$$

We use Random Forest for the first stage with $n=200$ and max depth 5. For comparison, we compute standard errors without clustering. The question of clustered standard errors with DoubleML will be addressed in the last section.
"""
